...
export default function HeroCurtain() {
  ...
  const [ribbonDown, setRibbonDown] = useState(false)
  const [exiting, setExiting] = useState(false)

  // 响应 sessionStorage 'curtain' → 幕布上滑
  useEffect(() => {
    if (sessionStorage.getItem('curtain') === '1') {
      sessionStorage.removeItem('curtain')
      setRibbonDown(true)
      setTimeout(() => setExiting(true), 20)
    }
  }, [pathname])
  ...
  if (isHome === false && exiting) {
    // 不是在首页且正在 exit → 返回 null，让幕布下面渲染的内页可见
  }

  return (
    <div className="fixed inset-0 z-[9999] bg-black overflow-hidden"
      style={{
        transform: exiting ? 'translateY(-105vh)' : 'translateY(0)',
        transition: exiting ? 'transform 0.5s cubic-bezier(0.65,0,0.35,1)' : 'none',
        pointerEvents: isHome ? 'auto' : (exiting ? 'none' : 'none'),
        display: (!isHome && !exiting) ? 'none' : 'block',
      }}>
      {/* ═══ 缎带 ═══ */}
      <div style={{
        position:'fixed', right:28, top:0, zIndex:1000, width:28,
        height: ribbonDown ? '70vh' : '110px',
        transition: ribbonDown ? 'height 0.5s cubic-bezier(0.34,1.56,0.64,1)' : 'none',
        pointerEvents:'none', overflow:'hidden',
      }}>
        <div style={{width:28,minHeight:'100%',background:'linear-gradient(180deg,#c8a888,#b89878 40%,#c8a888 100%)',borderRadius:'0 0 6px 6px',boxShadow:'0 2px 8px rgba(80,40,20,0.12)'}}/>
      </div>
