...
export default function DiaryAssistPage() {
  ...
  return (<InnerLayout>
    ...
    {loading && <Card className="border-[var(--border)] bg-[var(--surface)] mb-4"><CardContent className="p-6"><div className="flex items-center gap-3"><div className="w-6 h-6 rounded-full border-2 border-[var(--primary)] border-t-transparent animate-spin"/><p className="text-[13px] text-[var(--text-muted)]">AI 正在根据学生档案扩写评语...</p></div></CardContent></Card>}
    {result && <Card className="border-[var(--border)] bg-[var(--surface)]"><CardContent className="p-6"><div className="flex items-center gap-2 mb-3"><Sparkles size={14} className="text-[var(--primary)]"/><span className="text-[12px] font-semibold text-[var(--primary)] tracking-wider">AI 生成结果</span></div><div className="p-4 rounded-xl bg-[var(--bg)] border border-[var(--border)]"><p className="text-[14px] text-[var(--text)] leading-relaxed whitespace-pre-line" style={{fontFamily:"var(--font-serif)"}}>{result}</p></div><div className="flex gap-2 mt-3"><button className="px-4 py-2 rounded-xl bg-[var(--primary)] text-white text-[12px] tracking-wider border-none cursor-pointer">保存评语</button><button className="px-4 py-2 rounded-xl border border-[var(--border)] text-[var(--text-secondary)] text-[12px] tracking-wider bg-transparent cursor-pointer">重新生成</button></div></CardContent></Card>}
    </div></InnerLayout>)
}
