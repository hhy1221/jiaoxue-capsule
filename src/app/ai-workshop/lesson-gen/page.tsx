...
const MOCK_PLAN = { goal:'了解本地茶叶的历史与制作工艺，培养对家乡文化的自豪感', sections:[{title:'认识红茶',time:'10分钟',desc:'展示红茶图片，提问"你们家里谁喝茶？"→引导学生分享',materials:'红茶样品、图片'},{title:'采茶故事',time:'10分钟',desc:'讲本地采茶的故事，请学生模仿采茶动作，活跃气氛',materials:'无'},{title:'泡茶体验',time:'12分钟',desc:'老师现场泡茶，每组分一小杯品尝。安全提醒：水很烫！',materials:'热水壶、一次性杯子、茶叶'},{title:'画茶',time:'8分钟',desc:'每人画一幅"我心中的茶园"，下课贴在墙上',materials:'画纸、彩笔'}],interactions:['选择题: 本地最有名的是什么茶？A绿茶 B红茶 C花茶 (答案:B)','开放题: 你家里有谁爱喝茶？他们怎么喝的？','动作题: 谁来模仿一下采茶的动作？'],safety:'泡茶环节务必由老师操作，禁止学生触碰热水壶。确保水温适宜后再给学生品尝。',homework:'回家问爷爷奶奶：我们这里种茶有多久了？明天来分享'}
...
export default function LessonGenPage() {
  ...
  return (<InnerLayout>
    ...
    <Card className="border-[var(--border)] bg-[var(--surface)] mb-6"><CardContent className="p-6"><div className="flex gap-3 flex-wrap"><input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="例: 本地茶叶 面向2-4年级" className="flex-1 min-w-[200px] px-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--bg)] text-[13px] text-[var(--text)] outline-none focus:border-[var(--primary)] transition-colors"/><button onClick={()=>{setLoading(true);setTimeout(()=>{setResult(MOCK_PLAN);setLoading(false)},1500)}} disabled={!topic.trim()||loading} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[var(--primary)] text-white border-none cursor-pointer text-[13px] font-medium disabled:opacity-50"><Sparkles size={14}/>生成课件</button></div></CardContent></Card>
    {loading&&<Card className="border-[var(--border)] bg-[var(--surface)] mb-4"><CardContent className="p-6"><CastingCircle active={true} label="正在设计课程大纲…" /></CardContent></Card>}
    {result&&<InkReveal show={!!result}><Card className="border-[var(--border)] bg-[var(--surface)]"><CardContent className="p-6"><h3 className="text-[16px] font-semibold text-[var(--text)] mb-1">课程目标</h3><p className="text-[13px] text-[var(--text-secondary)] mb-4">{result.goal}</p>
