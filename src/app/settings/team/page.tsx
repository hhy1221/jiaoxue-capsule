...
export default function TeamSettingsPage() {
  ...
  return (<InnerLayout>
    ...
    <Card className="border-[var(--border)] bg-[var(--surface)]"><CardContent className="p-6"><h3 className="text-[14px] font-semibold text-[var(--text)] mb-4">数据导出</h3><p className="text-[12px] text-[var(--text-muted)] mb-3">导出全部队伍数据，包括学生档案、评语记录、照片、临别信等</p><div className="flex gap-3 flex-wrap"><button className="px-4 py-2 rounded-xl bg-[var(--primary)] text-white text-[12px] tracking-wider border-none cursor-pointer" onClick={()=>toast('全部数据导出中，请稍候…','success')}>导出全部数据</button><button className="px-4 py-2 rounded-xl border border-[var(--border)] text-[var(--text-secondary)] text-[12px] tracking-wider bg-transparent cursor-pointer" onClick={()=>toast('学生数据导出中，请稍候…','success')}>导出学生数据</button></div></CardContent></Card>
    </div></InnerLayout>)
}
