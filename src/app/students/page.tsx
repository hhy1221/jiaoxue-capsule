...
export default function StudentsPage() {
  return (<InnerLayout>
    <header className="flex items-center justify-between pb-[22px] mb-5 flex-wrap gap-3 relative" style={{borderBottom:'1.5px solid rgba(180,160,130,0.25)'}}>
      ...
      <div className="flex gap-2 flex-wrap"><button className="picture-book-btn" style={{fontSize:11}}><Search size={14}/> 搜索</button><button className="picture-book-btn primary" style={{fontSize:11}}><Plus size={14}/> 添加学生</button></div>
    ...
    <div className="grid grid-cols-[repeat(auto-fill,minmax(270px,1fr))] gap-5">
      {MOCK_STUDENTS.map((s,i)=>(<Link key={s.id} href={`/students/${s.id}`} className="no-underline" style={{transform:`rotate(${i%2===0?'-0.35deg':'0.25deg'})`}}>
    ...
    <button className="add-new-btn mt-6">＋ 新建学生档案</button>
  </InnerLayout>)
}
